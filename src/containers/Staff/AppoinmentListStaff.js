...
import { Button, Modal, ModalHeader, ModalFooter, FormGroup, Label, Input } from 'reactstrap';
...
import { getAllAppointment, deleteAppointment, updateAppointment } from '../graphql-client/appointment-queries.js';
        ...
        if (emailsend.called === true) {
            if (emailsend.data) {
                if (emailsend.data.sendMail.errCode === '0') {
                    ...
                    updateappointment({
                        variables: {
                            idApp: emailsend.data.sendMail.idApp,
                            name: null,
                            email: null,
                            phone: null,
                            product: null,
            ...
            <Modal isOpen={modal} toggle={toggle} size="lg" >
                ...
                <ModalFooter>
                    <Button color="primary" onClick={() => { HandleSendMail() }} >
                        {load ? <CircularProgress color="warning" /> : 'send'}
                    </Button>{' '}
                ...
                return (
                    <tr key={index}>
                        <>
                            ...
                            <td >
                                <button><FontAwesomeIcon className="btn-send" icon={['fas', appointment.check ? 'fa-square-check' : 'fa-paper-plane']} onClick={GetAppointmentSendMail.bind(this, appointment)} /></button>{' '}
